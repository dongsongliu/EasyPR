...
public class PlateLocate {
     ...
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(PlateLocate.class);
    ...
    public static final int DEFAULT_VERIFY_MAX = 24;

    final float DEFAULT_ERROR = 0.9f;//0.6f
    final float DEFAULT_ASPECT = 3.75f;
    // 角度判断所用常量
    public static final int DEFAULT_ANGLE = 60;//30
    ...
    public Vector<Mat> plateLocate(Mat src) {
        ...
        if (debug) {
            imwrite("target/debug_GaussianBlur.jpg", src_blur);
        }

        // Convert it to gray 将图像进行灰度化
        cvtColor(src_blur, src_gray, CV_RGB2GRAY);
        if (debug) {
            imwrite("target/debug_gray.jpg", src_gray);
        }
        ...
        if (debug) {
            imwrite("target/debug_Sobel.jpg", grad);
        }
        ...
        if (debug) {
            imwrite("target/debug_morphology.jpg", img_threshold);
        }
        ...
        if (debug) {
            ...
            imwrite("target/debug_Contours.jpg", result);
        }
        ...
        if (debug) {
            imwrite("target/debug_result.jpg", result);
        }
    ...
    private boolean verifySizes(RotatedRect mr) {
        ...
        int min = 34 * 8 * verifyMin; // minimum area
        int max = 34 * 8 * verifyMax; // maximum area
    ...
    private Mat showResultMat(Mat src, Size rect_size, Point2f center, int index) {
        ...
        if (debug) {
            imwrite("target/debug_crop_" + index + ".jpg", img_crop);
        }
        ...
        if (debug) {
            imwrite("target/debug_resize_" + index + ".jpg", resultResized);
        }
